...
class ControladorOrcamento:
    ...
    def alterar_orcamento(self):
        ...
        if o is not None:
            ...
            if atributo == "PERIODO" or atributo == "PERÍODO":
                ...
                categ = self.__controlador_geral.controlador_tipo_orcamento.achar_categoria(novo_valor)
    ...
    def adicionar_movimentacoes(self, movimentacao):
        for orcamento in self.__orcamentos:
            if movimentacao.data.year == orcamento.mes_ano.year and(movimentacao.data.month == orcamento.mes_ano.month
                                                                        and orcamento.tipo == movimentacao.categoria_movimentacao):
                ids_movimentacoes_adicionadas = {m.codigo for m in orcamento.movimentacoes}
                if movimentacao not in ids_movimentacoes_adicionadas:
                    orcamento.adicionar_movimentacao(movimentacao)
                    ids_movimentacoes_adicionadas.add(movimentacao.codigo)
                    return movimentacao
        return None
    ...
    def excluir_movimentacoes(self, movimentacao):
        for orcamento in self.__orcamentos:
            if movimentacao.data.year == orcamento.mes_ano.year and(movimentacao.data.month == orcamento.mes_ano.month
                                                                        and orcamento.tipo == movimentacao.categoria_movimentacao):
                orcamento.movimentacoes.remove(movimentacao)
                return movimentacao
        return None
